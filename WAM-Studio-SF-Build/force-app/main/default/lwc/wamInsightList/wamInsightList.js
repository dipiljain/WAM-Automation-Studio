...
export default class WamInsightList extends LightningElement {
  @api title = "Latest Insights";
  @api iconName = "standard:insights";
  @api items = [];
  @api loading = false;
  @api errorMessage;
  @api emptyMessage = "Insights will appear once AI analysis runs.";
  ...
  get normalizedItems() {
    return (this.items || []).map((item, index) => {
      const key = item.key || item.id || `insight-${index}`;
      ...
      return {
        ...
        nextStep: tone === "critical" ? "Escalate" : "Review",
